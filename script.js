document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateClock() {
        ...
        if (dateDisplay) dateDisplay.textContent = `> ${dayName} // ${monthName} ${day} // ${year}`;
    ...
    window.greetingText = "Welcome to the system, Devesh.";
        ...
        if (dragSrcEl !== this) {
            ...
            if (chrome.bookmarks) {
                chrome.bookmarks.get(targetId, (result) => {
                    if (result && result[0]) {
                        chrome.bookmarks.move(srcId, { parentId: result[0].parentId, index: result[0].index }, () => {
                            renderBookmarks();
                        });
                    }
                });
            ...
            chrome.bookmarks.getTree((bookmarkTreeNodes) => {
                ...
                if (bookmarkTreeNodes[0].children) {
                    ...
                    if (bookmarksBar && bookmarksBar.children) {
                        const topBookmarks = bookmarksBar.children.filter(node => node.url).slice(0, 16);
                        ...
                        if (topBookmarks.length > 0) {
                            topBookmarks.forEach(bookmark => {
                                ...
                                wrapper.addEventListener('dragend', function() { this.style.opacity = '1'; });
    ...
    let pomoTotalTime = (parseInt(pomodoroInput ? pomodoroInput.value : 25) || 25) * 60;
    ...
    async function fetchLeetCodeDaily() {
        ...
        try {
            ...
            lcContainer.innerHTML = '<p style="color:#ff003c; padding:10px;">[ERR] Could not fetch LeetCode Daily.</p>';
        }
    }
        ...
        termInput.addEventListener('keypress', async (e) => {
            if (e.key === 'Enter') {
                ...
                try {
                    switch(cmd) {
                        case 'help':
                            printTerm('> cmnds: help, clear, uuid, hash <str>, joke, b64e, b64d, hex, ip, time, timer', 'success');
                            break;
                        case 'clear':
                        ...
                        case 'uuid':
                            ...
                            break;
                        case 'hash':
                            if (!args) { printTerm('> Usage: hash <text>', 'error'); break; }
                        ...
                        case 'joke':
                            ...
                            try {
                                const jRes = await fetch('https://official-joke-api.appspot.com/jokes/programming/random');
                                const jData = await jRes.json();
                                if (jData && jData[0]) {
                                    printTerm(`> ${jData[0].setup}`, 'normal');
                                    setTimeout(() => printTerm(`> ... ${jData[0].punchline}`, 'success'), 1500);
                                }
                            } catch (e) { printTerm('> Could not fetch joke.', 'error'); }
                            break;
                        case 'time':
                            const d = new Date();
                            printTerm(`> Local: ${d.toLocaleString()}`, 'success');
                            printTerm(`> Epoch: ${Math.floor(d.getTime()/1000)}`, 'success');
                            break;
                        case 'timer':
                            const pTime = document.getElementById('pomodoro-time')?.textContent || '00:00';
                            const isRunning = document.querySelector('.hourglass-wrapper')?.classList.contains('pomo-running');
                            printTerm(`> Status: ${isRunning ? 'RUNNING' : 'PAUSED'}`, 'success');
                        ...
                        case 'b64e':
                            ...
                            break;
                        case 'b64d':
                            try { printTerm(atob(args), 'success'); } catch(e) { printTerm('> Invalid encoding', 'error'); }
                            break;
                        case 'hex':
                            printTerm(args.split('').map(c => c.charCodeAt(0).toString(16).padStart(2,'0')).join(' '), 'success');
                            break;
                        case 'ip':
    ...
    async function fetchQuote() {
        ...
        try {
            ...
            const fallbacks = [
                { c: "Logic will get you from A to B. Imagination will take you everywhere.", a: "Albert Einstein" },
                { c: "Simplicity is the ultimate sophistication.", a: "Leonardo da Vinci" },
                ...
                { c: "Programs must be written for people to read, and only incidentally for machines to execute.", a: "Abelson & Sussman" }
            ];
    ...
    function renderTasks() {
        ...
        tasks.forEach((task, index) => {
            ...
            const statusLabel = task.completed ? '<span style="color:#ff003c; font-size:0.8em; margin-right:5px;">[DONE]</span>' : '<span style="color:var(--primary-color); font-size:0.8em; margin-right:5px;">[PENDING]</span>';
            ...
            if (result.hackerPanelSizes) {
                panels.forEach((panel, index) => {
                    if (result.hackerPanelSizes[index]) {
                        if (result.hackerPanelSizes[index].width) panel.style.width = result.hackerPanelSizes[index].width;
                        if (result.hackerPanelSizes[index].height) panel.style.height = result.hackerPanelSizes[index].height;
                    }
                });
            }
        });
        ...
        chrome.storage.local.get(['hackerTasks'], (result) => {
            if (result.hackerTasks) {
                tasks = result.hackerTasks;
                 ...
                 tasks = [{ text: "Initialize system", completed: false }, { text: "Breach mainframe", completed: false }];
                 saveTasks();
            }
            renderTasks();
            processDailyRefresh();
    ...
    function applySettings(updateGreeting = false) {
        // Toggle Matrix
        document.getElementById('matrix').style.display = currentSettings.matrixBg ? 'block' : 'none';
        
        // Theme color
        document.documentElement.style.setProperty('--primary-color', currentSettings.color);
        
        // Clock style
        document.getElementById('clock-digital').style.display = currentSettings.clockStyle === 'digital' ? 'block' : 'none';
        document.getElementById('clock-analog').style.display = currentSettings.clockStyle === 'analog' ? 'block' : 'none';
        ...
        if(bmSection) bmSection.style.display = currentSettings.showBm ? 'flex' : 'none';
        if(dhSection) dhSection.style.display = currentSettings.showDh ? 'flex' : 'none';
        if(tdSection) tdSection.style.display = currentSettings.showTd ? 'flex' : 'none';

        if (updateGreeting && window.greetingText !== `Welcome to the system, ${currentSettings.name}.`) {
    ...
    function syncSettingsForm() {
        ...
        if(byId('setting-color')) byId('setting-color').value = currentSettings.color;
        if(byId('setting-clock')) byId('setting-clock').value = currentSettings.clockStyle;
        if(byId('setting-mode')) byId('setting-mode').value = currentSettings.dashboardMode || 'dev';
        if(byId('setting-matrix')) byId('setting-matrix').checked = currentSettings.matrixBg;
        if(byId('setting-show-bm')) byId('setting-show-bm').checked = currentSettings.showBm;
        if(byId('setting-show-dh')) byId('setting-show-dh').checked = currentSettings.showDh;
        if(byId('setting-show-td')) byId('setting-show-td').checked = currentSettings.showTd;
    }
    ...
    if (fabBtn) fabBtn.addEventListener('click', () => settingsModal.classList.remove('modal-hidden'));
    if (closeSettings) closeSettings.addEventListener('click', () => settingsModal.classList.add('modal-hidden'));
    ...
    const triggerSave = () => {
        currentSettings = {
            ...
            dashboardMode: document.getElementById('setting-mode') ? document.getElementById('setting-mode').value : 'dev',
    ...
    document.querySelectorAll('.settings-grid input, .settings-grid select').forEach(el => {
        ...
        if(el.type === 'color' || el.type === 'text') el.addEventListener('input', triggerSave);
    });
